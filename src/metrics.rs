...
///
/// This is useful for namespacing metrics in environments where multiple applications export to the same registry.
/// This should be called before the first request is processed; otherwise, it will not take effect.
pub fn set_prefix(prefix: &str) {
    PREFIXED_HTTP_REQUESTS_TOTAL.get_or_init(|| format!("{prefix}_http_requests_total"));
    PREFIXED_HTTP_REQUESTS_DURATION_SECONDS.get_or_init(|| format!("{prefix}_http_requests_duration_seconds"));
    PREFIXED_HTTP_REQUESTS_PENDING.get_or_init(|| format!("{prefix}_http_requests_pending"));
    PREFIXED_HTTP_RESPONSE_BODY_SIZE.get_or_init(|| format!("{prefix}_http_response_body_size"));
}

/// Adds one or more paths to the list of excluded paths for metrics collection.
///
/// Every URL that starts with one of the paths in the list is excluded. The `/metrics` endpoint is excluded by default.
